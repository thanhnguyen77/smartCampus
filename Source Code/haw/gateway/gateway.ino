...
void setup() {
  ...
  if (!LoRa.begin(frequency)) while (true)   Serial.println("lora loi");
  Serial.println("lora ahscbjasbc");
...
//  Serial.println();
  ...
  data["id"] = incomingMsgId;
  data["from"] = "0x"+String(sender, HEX);
  data["to"] = "0x"+String(recipient, HEX);
  data["length"] = String(incomingLength);
  data["Message"] = message;
//  data["Message"] = serialized(message);
  data["rssi"] = LoRa.packetRssi();
  data["Snr"] = LoRa.packetSnr();
  
  serializeJson(data, esp8266);
  esp8266.println();
  digitalWrite(14,1);
}
